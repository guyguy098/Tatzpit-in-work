...
async function fetchAiSum() {
	...
	populateNewsTicker(allArticles);
...
// ── News Ticker ──────────────────────────────────────────────────────────
function populateNewsTicker(articles) {
	const ticker = document.getElementById('aiSumTicker');
	const content = document.getElementById('aiSumTickerContent');
	if (!ticker || !content || !articles.length) return;

	// Pick top ~40 unique headlines, sorted newest first
	const seen = new Set();
	...
	// Build ticker items HTML (duplicate once for seamless loop)
	const buildItems = (list) => list.map(a => {
		const flag = getSourceFlag(a.sourceName);
		const safeUrl = /^https?:\/\//i.test(a.url) ? a.url.replace(/"/g, '&quot;') : '#';
		const safeTitle = (a.title || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
		const isRTL = /[\u0590-\u05FF\u0600-\u06FF]/.test(safeTitle.slice(0,5));
		return `<a class="news-ticker-item" href="${safeUrl}" target="_blank" rel="noopener">
			<span class="ticker-source">${flag} ${(a.sourceName||'').replace(/</g,'&lt;')}</span>
			<span class="ticker-sep">│</span>
			<span class="ticker-title" dir="${isRTL ? 'rtl' : 'ltr'}">${safeTitle}</span>
		</a>`;
	}).join('');

	// Duplicate items for seamless infinite scroll
	content.innerHTML = buildItems(items) + buildItems(items);

	// Adjust speed based on content count (more items = longer duration)
