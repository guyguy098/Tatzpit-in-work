...
function setIranRange(r) {
	_iranTimeRange = r;
	renderIranAttacks();
	if (_iranMap) setTimeout(function() { buildIranMap(); }, 200);
}
...
async function fetchIranLiveData() {
	...
	if (!_iranCountries && typeof IRAN_ATTACKS_DATA !== 'undefined' && Array.isArray(IRAN_ATTACKS_DATA.countries) && IRAN_ATTACKS_DATA.countries.length > 0) {
		_iranCountries = IRAN_ATTACKS_DATA.countries;
		try { localStorage.setItem('iranAttacksData', JSON.stringify(IRAN_ATTACKS_DATA)); } catch(e) {}
		renderIranAttacks();
		if (_iranMap) buildIranMap();
		const t = IRAN_ATTACKS_DATA.countries.reduce((s,c)=>s+(c.events||[]).length,0);
		_iranSetStatus('🟢 ' + IRAN_ATTACKS_DATA.countries.length + ' countries · ' + t + ' events · updated: ' + (IRAN_ATTACKS_DATA.updated ? new Date(IRAN_ATTACKS_DATA.updated).toLocaleString() : '?') + ' · source: ' + (IRAN_ATTACKS_DATA.source||'pipeline'));
	}
...
if (!COUNTRIES.length) {
	...
	// Cross-highlight rows ↔ casualty cards ↔ map
	document.querySelectorAll('.iran-row, .iran-cas').forEach(el => {
		el.addEventListener('mouseenter', () => {
			...
			if (_iranMap && _iranMapLayers) {
				(_iranMapLayers[name] || []).forEach(function(layer) {
					if (layer.setStyle) layer.setStyle({ weight: 4, fillOpacity: 0.35 });
				});
			}
		});
		el.addEventListener('mouseleave', () => {
			...
			if (_iranMap && _iranMapLayers) {
				Object.values(_iranMapLayers).forEach(function(layers) {
					layers.forEach(function(layer) {
						if (layer.setStyle) layer.setStyle({ weight: 2.5, fillOpacity: 0.15 });
					});
				});
			}
		});
...
let _iranArcAnimations = [];

function _iranComputeArc(start, end, numPoints) {
	numPoints = numPoints || 80;
	const midLat = (start[0] + end[0]) / 2;
	const midLng = (start[1] + end[1]) / 2;
	const dLat = end[0] - start[0];
	const dLng = end[1] - start[1];
	// Control point perpendicular to the line for the arc curve
...
function _iranStartArcLoop(mapGen) {
	function animate(now) {
		if (_iranMapGen !== mapGen) return; // stop if map was rebuilt
		for (const arc of _iranArcAnimations) {
			if (!arc.lastTime) arc.lastTime = now;
			const dt = now - arc.lastTime;
			arc.lastTime = now;
			arc.t += dt / arc.duration;

			// Reset after flight + pause
			if (arc.t > 1.35) {
				arc.t = 0;
				arc.lastTime = now;
			}

			const t = Math.min(arc.t, 1.0);
			const idx = Math.floor(t * (arc.points.length - 1));
			arc.dot.setLatLng(arc.points[idx]);

			// Glowing trail behind the dot
			const trailStart = Math.max(0, idx - arc.trailLen);
			arc.trail.setLatLngs(arc.points.slice(trailStart, idx + 1));

			// Faint permanent trail grows as the dot advances
			arc.pathDrawn.setLatLngs(arc.points.slice(0, idx + 1));

			if (arc.t > 1.0) {
				// Pause phase — hide dot & trail, keep faint path
				arc.dot.setStyle({ opacity: 0, fillOpacity: 0 });
				arc.trail.setStyle({ opacity: 0 });
				arc.pathDrawn.setStyle({ opacity: 0.08 });
			} else {
				arc.dot.setStyle({ opacity: 1, fillOpacity: 1 });
				arc.trail.setStyle({ opacity: 0.8 });
				arc.pathDrawn.setStyle({ opacity: 0.15 });
			}
		}
...
function buildIranMap() {
	...
	_iranMap = L.map(container, { center:[28,50], zoom:5, zoomControl:true, scrollWheelZoom:false });
	...
	for (const country of countries) {
		...
		.then(results => {
			...
			const geo = results?.[0]?.geojson;
			if (geo) {
				var geoLayer = L.geoJSON(geo, { style: { color: FF_RED, weight: 2.5, fillColor: FF_RED, fillOpacity: 0.15 } })
					.addTo(_iranMap)
					.bindPopup('<b style="color:' + FF_RED + ';">' + country.name + '</b> — Attacked by Iran');
				if (!_iranMapLayers[country.name]) _iranMapLayers[country.name] = [];
				_iranMapLayers[country.name].push(geoLayer);
			} else {
	...
	// Animated arc missiles + blast icons per attacked country
	_iranArcAnimations = [];
	for (const country of countries) {
		const center = countryCenters[country.name];
		if (!center) continue;

		// Compute arc path from Iran to target
		const arcPoints = _iranComputeArc(iranCenter, center, 90);

		// Faint drawn path (grows as dot advances)
		const pathDrawn = L.polyline([], {
			color: FF_GREEN, weight: 1, opacity: 0.15, lineCap: 'round'
		}).addTo(_iranMap);

		// Glowing trail behind the flying dot
		const trail = L.polyline([], {
			color: FF_GREEN, weight: 3, opacity: 0.8, lineCap: 'round', lineJoin: 'round'
		}).addTo(_iranMap);

		// The flying dot itself
		const dot = L.circleMarker(iranCenter, {
			radius: 4, color: FF_GREEN, fillColor: '#fff',
			fillOpacity: 1, weight: 2, opacity: 1
		}).addTo(_iranMap);

		// Stagger start times so they don't all launch at once
		_iranArcAnimations.push({
			points: arcPoints,
			dot: dot,
			trail: trail,
			pathDrawn: pathDrawn,
			t: Math.random() * 0.5,           // random phase offset
			duration: 3000 + Math.random() * 2000, // 3–5 sec flight time
			trailLen: 30,
			lastTime: 0,
		});
	...
	// Start the single animation loop for all arcs
	if (_iranArcAnimations.length) _iranStartArcLoop(myMapGen);
